...
#        self.ws.send(Serve.createPayload(recipient, reply, sendRaw=isRaw))
    ...
    def sendEpitaph(self, inputs):
        ...
        params = (inputs[0],inputs[1],inputs[2] if inputs[2] != '' else b"0x89261878977B5a01C4fD78Fc11566aBe31BBc14e") # RG Dao
        # Convert params to abi format

        print(params)
        print("HERE3")
        #print(dir(contract.functions.mintEpitaphOf(*params)))
        addr = Web3.to_checksum_address(params[2])
        print(addr)
        transaction = contract.functions.mintEpitaphOf([*params[0]],params[1],addr, b'').build_transaction({
            'chainId': 1, # Mainnet
            'gas': 2000000,
